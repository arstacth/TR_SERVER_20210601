...
namespace LocalCommons.Network
{
	public abstract class IConnection : IDisposable
	{
		...
		private static BufferPool m_RecvBufferPool = new BufferPool("Receive", 1024, 65535);
		...
		private void ProceedReceiving(SocketAsyncEventArgs e)
		{
			...
			int num = packetReader.ReadLEUInt16() - 3;
			ushort num2 = 2;
			do
			{
				...
				catch (ArgumentException ex)
				{
					...
					Log.Error("[ArgumentException] packets: {0}\r\noffset: {1}, size: {2}, length: {3}, ip,:{4} buffer:{5}", ex.Message, num2, size, num, m_Address, text2.Substring(0, Math.Min(512, text2.Length)));
					this.DisconnectedEvent?.Invoke(this, EventArgs.Empty);
					break;
				}
				catch (Exception ex2)
				{
					Log.Error("Errors when parsing glued packets : {0}", ex2.ToString());
				}
				num2 = (ushort)(num2 + (ushort)(num + 1));
				if (num2 < size)
				{
					packetReader.Offset = num2;
					num2 = (ushort)(num2 + 2);
