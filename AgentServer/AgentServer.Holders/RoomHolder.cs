using System;
using System.Collections.Concurrent;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using AgentServer.Structuring.Item;
using AgentServer.Structuring.Room;
using MySql.Data.MySqlClient;
using Serilog;

namespace AgentServer.Holders
{
	public static class RoomHolder
	{
		public static ConcurrentDictionary<int, RoomKindInfo> RoomKindInfos { get; } = new ConcurrentDictionary<int, RoomKindInfo>();


		public static ConcurrentDictionary<int, List<ItemAttr>> RoomKindAttr { get; } = new ConcurrentDictionary<int, List<ItemAttr>>();


		public static ConcurrentDictionary<int, RoomKind_UserMinMax> RoomKindPlayerNum { get; } = new ConcurrentDictionary<int, RoomKind_UserMinMax>();


		public static void LoadRoomKindInfo()
		{
			using (MySqlConnection mySqlConnection = new MySqlConnection(Conf.Connstr))
			{
				mySqlConnection.Open();
				using MySqlCommand mySqlCommand = new MySqlCommand(string.Empty, mySqlConnection);
				mySqlCommand.Parameters.Clear();
				mySqlCommand.CommandType = CommandType.StoredProcedure;
				mySqlCommand.CommandText = "usp_getRoomKindID";
				using MySqlDataReader mySqlDataReader = mySqlCommand.ExecuteReader();
				while (mySqlDataReader.Read())
				{
					RoomKindInfo value = new RoomKindInfo
					{
						GameMode = Convert.ToInt32(mySqlDataReader["GameMode"]),
						Channel = Convert.ToInt32(mySqlDataReader["Channel"]),
						PlayMode = Convert.ToInt32(mySqlDataReader["PlayMode"])
					};
					RoomKindInfos.TryAdd(Convert.ToInt32(mySqlDataReader["RoomKindID"]), value);
				}
			}
			Log.Information("Load RoomKindInfo Count: {0}", RoomKindInfos.Count());
			RoomKindAttr.Clear();
			using (MySqlConnection mySqlConnection2 = new MySqlConnection(Conf.Connstr))
			{
				mySqlConnection2.Open();
				using MySqlCommand mySqlCommand2 = new MySqlCommand(string.Empty, mySqlConnection2);
				mySqlCommand2.Parameters.Clear();
				mySqlCommand2.CommandType = CommandType.StoredProcedure;
				mySqlCommand2.CommandText = "usp_getRoomKindAttr";
				using MySqlDataReader mySqlDataReader2 = mySqlCommand2.ExecuteReader();
				while (mySqlDataReader2.Read())
				{
					int @int = mySqlDataReader2.GetInt32("roomKind");
					ItemAttr Attr = new ItemAttr
					{
						Attr = mySqlDataReader2.GetUInt16("AttrType"),
						AttrValue = mySqlDataReader2.GetFloat("AttrValue")
					};
					RoomKindAttr.AddOrUpdate(@int, new List<ItemAttr> { Attr }, delegate(int k, List<ItemAttr> v)
					{
						v.Add(Attr);
						return v;
					});
				}
			}
			Log.Information("Load RoomKindAttr Count: {0}", RoomKindAttr.Count);
			RoomKindPlayerNum.Clear();
			try
			{
				using (MySqlConnection mySqlConnection3 = new MySqlConnection(Conf.Connstr))
				{
					mySqlConnection3.Open();
					using MySqlCommand mySqlCommand3 = new MySqlCommand("select * from EssenCompetitionEvent_PlayerCount where fdUse = 1;", mySqlConnection3);
					mySqlCommand3.Parameters.Clear();
					using MySqlDataReader mySqlDataReader3 = mySqlCommand3.ExecuteReader();
					while (mySqlDataReader3.Read())
					{
						int int2 = mySqlDataReader3.GetInt32("fdRoomKindID");
						int int3 = mySqlDataReader3.GetInt32("fdMin");
						int int4 = mySqlDataReader3.GetInt32("fdMax");
						RoomKind_UserMinMax value2 = new RoomKind_UserMinMax
						{
							MinUser = int3,
							MaxUser = int4
						};
						RoomKindPlayerNum.TryAdd(int2, value2);
					}
				}
				Log.Information("Load CompetitionEven PlayerNum Done!");
			}
			catch (Exception ex)
			{
				Log.Error("Load CompetitionEven PlayerNum Error:{0}", ex.ToString());
			}
		}
	}
}
